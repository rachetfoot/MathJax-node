...
(function() {
  ...
  Module._load = function _load(file, parent, core, main) {
	...
	loadModule(body, file, module.exports, module, module.require, module.filename, dir);
    module.loaded = true;
    module.main = main;
  ...
  function Require(id, parent) {
    ...
    if (parent && parent.id) {
      console.debug("Loading " + id + " from " + parent.id);
    } else {
      console.debug("Loading " + id + " from null");
    }
  ...
  function _readFile(filename, core) {
    try {
      ...
      throw new ModuleError("Cannot read file ["+filename+"]: "+e.message, "IO_ERROR", e);
    }
  }
