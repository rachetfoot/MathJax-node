...
setTimeout = jsdom.setTimeout;
clearTimeout = jsdom.clearTimeout;
var pathResolve = jsdom.pathResolve;
jsdom = jsdom.jsdom;
...
var MathJaxPath = require.resolve("../MathJax/unpacked/MathJax.js");
MathJaxPath = MathJaxPath.path || MathJaxPath;
...
document = window = content = html = null; // the DOM elements
MathJax = {};
...
function ConfigureMathJax() {
  console.debug("Configuring MathJax");

  var svgblacker;
  if (java.lang.System.getProperty("java.vendor").contains("oracle")) {
    svgblacker = 1;
  } else {
    svgblacker = 0;
  }
  ...
  window.MathJax = {
    ...
    extensions: ["tex2jax.js","mml2jax.js","asciimath2jax.js","toMathML.js", "Safe.js"],
    TeX: {extensions: new Array("AMSmath.js","AMSsymbols.js","autoload-all.js","color.js", "mhchem.js")},
    tex2jax: {inlineMath: [['$','$'],['\\(','\\)']], displayMath: [ ['$$','$$'], ['\\[','\\]'] ], preview:"none"},
    mml2jax: {preview:"none"},
    asciimath2jax: {preview:"none"},
    SVG: {useFontCache: true, useGlobalCache: false, blacker: svgblacker, EqnChunk: 1000000, EqnDelay: 0, scale: 100},
...
function GetMML(result) {
  ...
  try {result.mml = jax.root.toMathML('',jax)} catch(err) {
    ...
    return MathJax.Callback.After(Array(GetMML,result),err.restart);
  }
}
...
function GetSVG(result) {
  ...
  svg.style.removeProperty("margin-right");
...
function StartQueue() {
  ...
  var math = data.math;
  if ((data.format !== "MathML") && (data.format !== "NoDelims"))
  ...
  console.debug("MathJax typesetting is queued, waiting for result.")
}
...
function TypesetDone(result) {
  console.debug("Typeset is done, update MathJax state");
...
function RestartMathJax() {
  if (timer) {
    ...
    timer = window = document = html = content = null;
	MathJax = {};
...
exports.typeset = function (data,callback) {
  if (!data || !data.math) {
    callback({errors: "No latex was provided to typeset"});
    return;
  }
